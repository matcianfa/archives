...
import numpy as np
from numba import jit, vectorize, guvectorize, float64, complex128, int32, float32  #A installer avec conda. Permet d'optimiser enormément la vitesse de calcul
...
@jit(int32(complex128, complex128,int32),nopython=True)
def julia_opti(z,c,max_iter):
    """
...
@guvectorize([(complex128[:],complex128, int32[:], int32[:,:])], '(n),(),(p)->(n,p)',target='parallel')
def julia_numpy(z,c, maxit, output):
    maxiter=maxit[0]
    for i in range(z.shape[0]):
        n = julia_opti(z[i],c,maxiter)
        output[i,0] = n%255
        output[i,1] = 255
        if n < maxiter:
            output[i,2] = 255
...
def julia_numpy_nb(z,c, maxiter, output):
    for i in range(z.shape[0]):
        n = julia_opti(z[i],c,maxiter)
        if n < maxiter: val = 255
        else : val =0
        output[i] = val
...
def creer_image(max_iter=MAX_ITER):
    ...
    Z = X + Y[:,None]*1j
    im = Image.fromarray(julia_numpy(Z,complex(CR,CI),np.array([max_iter,0,0])).astype(np.int8), "HSV")

    im.convert('RGB').save('output.png', 'PNG')

def creer_image_NB(max_iter=MAX_ITER):
    ...
    Z = X + Y[:,None]*1j
    im = Image.fromarray(julia_numpy_nb(Z,complex(CR,CI),max_iter).astype(np.int8))

    im.convert('L').save('outputNB.png', 'PNG')
...
def lancer(cr=0.285,ci=0.01):
    ...
    mandelbrot=False # Pour afficher l'ensemble de mandelbrot
    ...
    while continuer :
                ...
                if event.key == K_ESCAPE : # On réinitialise le zoom
                    ...
                    for case in  cases_a_cocher[3:0:-1]:
                        if case.cochee :
                            ...
                            case.cliqué()
                            break
                if event.unicode=='m' : # Si on appuie sur m on affiche mandelbrot
                    X_MIN,Y_MIN,X_MAX = -2,-1,1
                    Y_MAX=Y_MIN+HEIGHT*(X_MAX-X_MIN)/WIDTH
                    mandelbrot = True
            ...
            if event.type== MOUSEBUTTONUP: # quand je relache le bouton
                if event.button == 1: # 1= clique gauche
                    ...
                    if pygame.key.get_mods() & pygame.KMOD_SHIFT:
                        ...
                        # On désactive l'affichage de l'ensemble de Mandelbrot
                        mandelbrot = False
                        # On réaffiche l'image
                        for case in  cases_a_cocher[3:0:-1]:
                            if case.cochee :
                ...
                if event.button == 3 : # Si on relache le bouton droit
                    ...
                    a,b = position_depart
                    c,d = event.pos
                    if min(abs(a-c),abs(b-d))>3 : # Si on n'a pas laché trop tot pour avoir un rectangle pas trop petit
        ...
        # On affiche les images calculées
        if mandelbrot :
            fenetre.blit(pygame.image.load("mandelbrot.png").convert(),(0,0))
        elif cases_a_cocher[2].cochee or cases_a_cocher[3].cochee :
        ...
        fenetre.blit(font.render("Appuyer sur M pour afficher l'ensemble de Mandelbrot",1,(0,0,0)),(10,HEIGHT-90))
...
Rajouter l'ensemble de Mandelbrot pour mieux selectionner les bons ensembles de Julia
"""