...
@jit(int32(complex128, int32),nopython=True)
def mandelbrot_opti(c,max_iter):
    """
...
@guvectorize([(complex128[:], int32[:], int32[:,:])], '(n),(p)->(n,p)',target='parallel')
def mandelbrot_numpy(c, maxit, output):
    # np.dstack((a,b,c))
    maxiter=maxit[0]
    for i in range(c.shape[0]):
        n = mandelbrot_opti(c[i],maxiter)
        output[i,0] = n%255
        output[i,1] = 255
        if n < maxiter: 
            output[i,2] = 255
...
def mandelbrot_numpy_nb(c, maxiter, output):
    for i in range(c.shape[0]):
        n = mandelbrot_opti(c[i],maxiter)
...
def creer_image_NB(max_iter=MAX_ITER):
    ...
    C = X + Y[:,None]*1j
    im = Image.fromarray(mandelbrot_numpy_nb(C,max_iter).astype(np.int8))
...
def creer_image(max_iter=MAX_ITER):
    ...
    Y = np.linspace(Y_MIN, Y_MAX, HEIGHT,dtype=np.float64)
    C = X + Y[:,None]*1j
    im = Image.fromarray(mandelbrot_numpy(C,np.array([max_iter,0,0])).astype(np.int8), "HSV")
...
def lancer():
    ...
    #cases_a_cocher.append(Case_a_cocher((10,70),"Afficher vitesse de divergence (HD)",cochee=False,fonction=lambda :creer_image(10000)))
